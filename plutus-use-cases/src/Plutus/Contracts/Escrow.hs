...
redeem inst escrow = mapError (review _EscrowError) $ do
    ...
    else if foldMap Tx.getTxOutValue unspentOutputs `lt` targetTotal escrow
...
refund inst escrow = do
    ...
    let flt _ ciTxOut = either id Ledger.datumHash (Tx.scOutDatum ciTxOut) == Ledger.datumHash (Datum (PlutusTx.toBuiltinData $ Ledger.pubKeyHash pk))
...
payRedeemRefund params vl = do
    let inst = typedValidator params
        go = do
            ...
            let presentVal = foldMap Tx.getTxOutValue cur
