...
tests :: TestTree
tests =
    let runOld :: Slot -> String -> TracePredicate -> EmulatorTrace () -> _
        runOld sl = checkPredicateOptionsOld (defaultCheckOptions & maxSlot .~ sl & minLogLevel .~ Debug)
        runV2 :: Slot -> String -> TracePredicate -> EmulatorTrace () -> _
        runV2 sl = checkPredicateOptionsV2 (defaultCheckOptions & maxSlot .~ sl & minLogLevel .~ Debug)

        checkOld :: Slot -> String -> Contract () Schema ContractError () -> _ -> _
        checkOld sl nm contract pred = runOld sl nm (pred contract) (void $ activateContract w1 contract tag)
        checkV2 :: Slot -> String -> Contract () Schema ContractError () -> _ -> _
        checkV2 sl nm contract pred = runV2 sl nm (pred contract) (void $ activateContract w1 contract tag)
    ...
    in
    testGroup "contracts"
        [ checkOld 1 "awaitSlot" (void $ awaitSlot 10) $ \con ->
            waitingForSlot con tag 10
        , checkV2 1 "awaitSlot" (void $ awaitSlot 10) $ \con ->
            waitingForSlot con tag 10

        , checkOld 1 "selectEither" (void $ awaitPromise $ selectEither (isSlot 10) (isSlot 5)) $ \con ->
            waitingForSlot con tag 5
        , checkV2 1 "selectEither" (void $ awaitPromise $ selectEither (isSlot 10) (isSlot 5)) $ \con ->
            waitingForSlot con tag 5

        , checkOld 1 "both" (void $ awaitPromise $ Con.both (isSlot 10) (isSlot 20)) $ \con ->
            waitingForSlot con tag 10
        , checkV2 1 "both" (void $ awaitPromise $ Con.both (isSlot 10) (isSlot 20)) $ \con ->
            waitingForSlot con tag 10

        , checkOld 1 "both (2)" (void $ awaitPromise $ Con.both (isSlot 10) (isSlot 20)) $ \con ->
            waitingForSlot con tag 20
        , checkV2 1 "both (2)" (void $ awaitPromise $ Con.both (isSlot 10) (isSlot 20)) $ \con ->
            waitingForSlot con tag 20

        , checkOld 1 "watchAddressUntilSlot" (void $ watchAddressUntilSlotOld someAddress 5) $ \con ->
            waitingForSlot con tag 5
        , checkV2 1 "watchAddressUntilSlot" (void $ watchAddressUntilSlotOld someAddress 5) $ \con ->
            waitingForSlot con tag 5

        , checkOld 1 "endpoint" (void $ awaitPromise $ endpoint @"ep" pure) $ \con ->
            endpointAvailable @"ep" con tag
        , checkV2 1 "endpoint" (void $ awaitPromise $ endpoint @"ep" pure) $ \con ->
            endpointAvailable @"ep" con tag

        , checkOld 1 "forever" (forever $ awaitPromise $ endpoint @"ep" pure) $ \con ->
            endpointAvailable @"ep" con tag
        , checkV2 1 "forever" (forever $ awaitPromise $ endpoint @"ep" pure) $ \con ->
            endpointAvailable @"ep" con tag
        ...
        , let theContract :: Contract () Schema ContractError () = void $ awaitPromise $ endpoint @"1" @Int pure .> endpoint @"2" @Int pure
          in runOld 1 "call endpoint (1)"
                (endpointAvailable @"1" theContract tag)
                (void $ activateContract w1 theContract tag)
        , let theContract :: Contract () Schema ContractError () = void $ awaitPromise $ endpoint @"1" @Int pure .> endpoint @"2" @Int pure
          in runV2 1 "call endpoint (1)"
                (endpointAvailable @"1" theContract tag)
                (void $ activateContract w1 theContract tag)

        , let theContract :: Contract () Schema ContractError () = void $ awaitPromise $ endpoint @"1" @Int pure .> endpoint @"2" @Int pure
          in runOld 1 "call endpoint (2)"
        ...
        , let theContract :: Contract () Schema ContractError () = void $ submitTx mempty >> watchAddressUntilSlotOld someAddress 20
          in runOld 1 "submit tx"
                (waitingForSlot theContract tag 20)
                (void $ activateContract w1 theContract tag)
        , let theContract :: Contract () Schema ContractError () = void $ submitTx mempty >> watchAddressUntilSlotOld someAddress 20
          in runV2 1 "submit tx"
                (waitingForSlot theContract tag 20)
                (void $ activateContract w1 theContract tag)
        ...
        , let smallTx = Constraints.mustPayToPubKey (Crypto.pubKeyHash $ walletPubKey (Wallet 2)) (Ada.lovelaceValueOf 10)
          ...
          in runOld 3 "handle several blockchain events"
        ...
        , let smallTx = Constraints.mustPayToPubKey (Crypto.pubKeyHash $ walletPubKey (Wallet 2)) (Ada.lovelaceValueOf 10)
              theContract :: Contract () Schema ContractError () = submitTx smallTx >>= awaitTxConfirmed . Ledger.txId >> submitTx smallTx >>= awaitTxConfirmed . Ledger.txId
          in runV2 3 "handle several blockchain events"
                (walletFundsChange w1 (Ada.lovelaceValueOf (-20))
                    .&&. assertNoFailedTransactions
                    .&&. assertDone theContract tag (const True) "all blockchain events should be processed")
                (void $ activateContract w1 theContract tag >> Trace.waitUntilSlot 3)
        ...
        , let theContract :: Contract () Schema ContractError () = void $ awaitUtxoProduced (walletAddress w2)
          in runOld 2 "await utxo produced"
        ...
        , let theContract :: Contract () Schema ContractError () = void (utxoAtOld (walletAddress w1) >>= awaitUtxoSpent . fst . head . Map.toList)
          in runOld 2 "await txout spent"
            (assertDone theContract tag (const True) "should receive a notification")
            (void $ do
                activateContract w1 theContract tag
                Trace.payToWallet w1 w2 (Ada.lovelaceValueOf 200)
        ...
        , runV2 1 "error handling & checkpoints"
